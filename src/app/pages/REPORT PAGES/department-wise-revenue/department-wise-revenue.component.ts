...
import { ClaimWiseCostingComponent } from '../claim-wise-costing/claim-wise-costing.component';
...
@Component({
  selector: 'app-department-wise-revenue',
  templateUrl: './department-wise-revenue.component.html',
  styleUrl: './department-wise-revenue.component.scss',
  providers: [ReportService, ReportEngineService],
})
export class DepartmentWiseRevenueComponent {
  ...
  detailDataCache: { [DepartmentID: number]: any[] } = {};
  ...
  async get_Datagrid_DataSource() {
    ...
    try {
      const response: any = await this.service
        .fetch_Department_Wise_Revenue_Data(formData)
        .toPromise();
      if (response.flag === '1') {
  ...
  async detailDataRowExpanding(e: any) {
    ...
    // Show loading
    this.dataGrid.instance?.beginCustomLoading?.('Loading sub-details...');
  ...
  onClickUpdateNewCptType = () => {
    ...
    console.log('CPT Master Data (Edit):', cptData);
    const {
      ID,
      CPTTypeID,
      CPTCode,
  ...
  onExporting(event: any) {
    console.log('datagrid event::', event);
...
@NgModule({
  ...
  providers: [],
  exports: [],
  declarations: [DepartmentWiseRevenueComponent],
})
export class DepartmentWiseRevenueModule {}
