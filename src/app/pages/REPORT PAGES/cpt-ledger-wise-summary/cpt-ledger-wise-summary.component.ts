...
@Component({
  selector: 'app-claim-wise-costing',
  templateUrl: './cpt-ledger-wise-summary.component.html',
  styleUrl: './cpt-ledger-wise-summary.component.scss',
  providers: [ReportService, ReportEngineService],
})
export class CptLedgerWiseSummaryComponent {
  ...
  detailDataCache: { [claimUID: number]: any[] } = {};
  subDetailDataCache: { [claimActivityUID: number]: any[] } = {};
  ...
  previousExpandedRowIndex: number | null = null;
  detailExpandedKey: any = null;
  subDetailExpandedKey: any = null;
  ...
  async get_Datagrid_DataSource() {
    ...
    try {
      const response: any = await this.service
        .fetch_CPT_Ledger_wise_summary_Data(formData)
        .toPromise();
      if (response.flag === '1') {
  ...
  async headerDataRowExpanding(e: any) {
    ...
    // Find row index of newly expanded row
    const newRowIndex = this.dataGrid.instance.getRowIndexByKey(e.key);

    // Store previous row index (for repaint)
    const previousIndex = this.previousExpandedRowIndex;

    // Update expanded key & previous index
    this.expandedRowKey = e.key;
    this.previousExpandedRowIndex = newRowIndex;

    // ⭐ Repaint old + new row (to clear / apply highlight)
    const rowsToRepaint = [];
    if (previousIndex !== null && previousIndex >= 0) {
      rowsToRepaint.push(previousIndex);
    }
    if (newRowIndex >= 0) {
      rowsToRepaint.push(newRowIndex);
    }

    if (rowsToRepaint.length > 0) {
      this.dataGrid.instance.repaintRows(rowsToRepaint);
    }

    const expandedKey = e.key;
    const CPTCode = expandedKey.CPTCode || e.data.CPTCode;

    this.FilteredDetailData = [];

    if (this.detailDataCache[CPTCode]) {
      this.FilteredDetailData = this.detailDataCache[CPTCode];
      this.originalDetailData = [...this.FilteredDetailData];
      return;
    }

      // === Show loading indicator ===
    this.detailsdatagrid.instance.beginCustomLoading('Loading...');
    ...
    try {
      ...
      if (response.flag === '1') {
        ...
        this.detailDataCache[CPTCode] = [...this.FilteredDetailData];

        this.detailsdatagrid.instance.endCustomLoading();
      }
    } catch (error) {
      notify({ message: 'Error loading detail data' }, 'error');
      this.detailsdatagrid.instance.endCustomLoading();
    }
  }
  ...
  async detailDataRowExpanding(e: any) {
    // --- collapse previous detail row ---
    const detailGrid = e.component; // the detail dx-data-grid instance
    if (this.detailExpandedKey && this.detailExpandedKey !== e.key) {
      detailGrid.collapseRow(this.detailExpandedKey);
    }

    // Store current key
    this.detailExpandedKey = e.key;

    // Your existing logic
    const CPTCode = e.key?.CptCode || e.data?.CptCode;
    const DepartmentID = e.key?.DepartmentID || e.data?.DepartmentID;
    const LedgerID = e.key?.LedgerID || e.data?.LedgerID;

    const cacheKey = `${CPTCode}_${DepartmentID}_${LedgerID}`;

    if (this.subDetailDataCache[cacheKey]) {
      this.FilteredSubDetailData = this.subDetailDataCache[cacheKey];
      return;
    }

    this.subDetailsGrid.instance.beginCustomLoading('Loading...');
    ...
    try {
      ...
      if (response?.flag === '1') {
        ...
        // Cache result
        this.subDetailDataCache[cacheKey] = [...this.FilteredSubDetailData];

        this.subDetailsGrid.instance.endCustomLoading();
      } else {
        notify({ message: response?.message || 'No data found' }, 'warning');
        this.subDetailsGrid.instance.endCustomLoading();
      }
    } catch (error) {
      console.error(error);
  ...
  onCellClick(e: any) {
    ...
    if (dataField === 'ClaimNumber') {
      ...
      console.log(this.clickedCellRowData, 'clickedCellRowData');
  ...
  onExporting(event: any) {
    const fileName = 'cpt_ledger_wise_summary';
...
@NgModule({
  ...
  providers: [],
  exports: [],
  declarations: [CptLedgerWiseSummaryComponent],
})
export class CptLedgerWiseSummaryModule {}
