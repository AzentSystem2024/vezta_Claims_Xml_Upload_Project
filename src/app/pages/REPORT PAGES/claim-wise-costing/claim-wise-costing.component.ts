...
@Component({
  ...
  templateUrl: './claim-wise-costing.component.html',
  styleUrl: './claim-wise-costing.component.scss',
  providers: [ReportService, ReportEngineService],
})
export class ClaimWiseCostingComponent {
  ...
  async get_Datagrid_DataSource() {
    ...
    try {
      const response: any = await this.service
        .fetch_Claim_wise_Costing_Data(formData)
        .toPromise();
      if (response.flag === '1') {
  ...
  async headerDataRowExpanding(e: any) {
    ...
    const claimUID = expandedKey.ClaimUID || e.data.ClaimUID;
    this.FilteredDetailData = [];

    // === Show loading indicator ===
    this.dataGrid.instance.beginCustomLoading('Loading...');
    // Use cached data if available
    if (this.detailDataCache[claimUID]) {
      this.FilteredDetailData = this.detailDataCache[claimUID];
      ...
      try {
        ...
        if (response.flag === '1') {
          ...
          // Cache detail data by ClaimUID
          this.detailDataCache[claimUID] = [...this.FilteredDetailData];
    ...
    // Set only expanded row in the grid
  ...
  async detailDataRowExpanding(e: any) {
    ...
    const claimActivityUID = e.key.ClaimActivityUID || e.data.ClaimActivityUID;
    const claimUID = e.key.ClaimUID || e.data.ClaimUID;

    // Filter and show only this row
    // const expandedRow = this.originalDetailData.find(
    //   (row) => row.ClaimActivityUID === claimActivityUID
    // );
    // this.FilteredDetailData = [expandedRow];

    // Use cached data if available
    if (this.subDetailDataCache[claimActivityUID]) {
      this.FilteredSubDetailData = this.subDetailDataCache[claimActivityUID];
      return;
    }
    ...
    const formData = {
      ...
      DateTo: this.reportengine.formatDate(this.To_Date_Value),
      ClaimUID: claimUID,
      ClaimActivityUID: claimActivityUID,
    };
    ...
    try {
      ...
      if (response.flag === '1') {
        ...
        // Cache sub-detail data by ClaimActivityUID
        this.subDetailDataCache[claimActivityUID] = [
  ...
  exportServerExcel = () => {
    ...
    const payload = {
      reportId: 'rpt-claim-wise-costing',
      facilityId: this.Facility_Value.join(','),
    ...
    this.masterService.exportReport(payload).subscribe({
      next: (blob: Blob) => {
        const fileName =
          `ClaimWiseCosting_${new Date().toISOString().slice(0, 19)}.xlsx`;
...
@NgModule({
  ...
  providers: [],
  exports: [],
  declarations: [ClaimWiseCostingComponent],
})
export class ClaimWiseCostingModule {}
