...
@Component({
  selector: 'app-claim-details-with-activity',
  templateUrl: './claim-details-with-activity.component.html',
  styleUrl: './claim-details-with-activity.component.scss',
  providers: [ReportService, ReportEngineService],
})
export class ClaimDetailsWithActivityComponent {
  ...
  detailCache: { [ClaimActivityUID: string]: any[] } = {};
  ...
  async get_Datagrid_DataSource() {
    ...
    try {
      const response: any = await this.service
        .fetch_Claim_Details_With_Activity_Data(formData)
        .toPromise();
  ...
  // ========== highlite cost department manually changed rows ========
  onRowPrepared(e: any) {
    if (e.rowType === 'data' && e.data?.CostDepartmentAllocationMode === 2) {
      e.rowElement.style.color = '#ff1500';
    }
  }
  ...
  async headerDataRowExpanding(e: any) {
    ...
    if (this.detailCache?.[ClaimActivityUID]) {
      this.FilteredDetailData = this.detailCache[ClaimActivityUID];
    } else {
      ...
      try {
        ...
        if (response.flag === '1') {
          ...
          this.detailCache[ClaimActivityUID] = detailData;
  ...
  onExporting(event: any) {
    const fileName = 'Cliam Details with activity';
  ...
  exportServerExcel = () => {
      ...
      const payload = {
        reportId: 'rpt-claim-details-with-activity',
        facilityId: this.Facility_Value.join(','),
      ...
      this.masterService.exportReport(payload).subscribe({
        next: (blob: Blob) => {
          const fileName =
            `ClaimDetailsWithActivity_${new Date().toISOString().slice(0, 19)}.xlsx`;
...
@NgModule({
  ...
  providers: [],
  exports: [],
  declarations: [ClaimDetailsWithActivityComponent],
})
export class ClaimDetailsWithActivityModule {}
